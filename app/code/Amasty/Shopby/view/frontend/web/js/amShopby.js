...
define([
    ...
    $.widget('mage.amShopbyFilterAbstract', {
                ...
                if (item.value.trim() != '' && item.value != '-1') {
                    ...
                    if (isSorting && item.name == 'amshopby[price][]') {
                        var values = item.value.split('-'),
                            ...
                            from = values[0] ? self.fixPriceForCurrency(parseFloat(values[0]), 'from', rate) : values[0],
                            to = values[1] ? self.fixPriceForCurrency(parseFloat(values[1]), 'to', rate) : values[1];
        ...
        fixPriceForCurrency: function (price, type, rate) {
            ...
            return price ? price.toFixed(4) : price;
        }
    });
    ...
    $.widget('mage.amShopbyFilterSlider', $.mage.amShopbyFilterAbstract, {
        ...
        onSyncChange: function (event, values) {
            ...
            if (value.length === 2) {
                ...
                var from = $.mage.amShopbyFilterAbstract.prototype.fixPriceForCurrency(parseFloat(from), 'from', rate),
    ...
    $.widget('mage.amShopbyFilterFromTo', $.mage.amShopbyFilterAbstract, {
        ...
        applyFilter: function (e) {
            ...
            var linkHref = this.options.url
                .replace('amshopby_slider_from', fromNew)
                .replace('amshopby_slider_to', toNew);
            this.apply(linkHref);
        ...
        onSyncChange: function (event, values) {
            ...
            if (value.length === 2) {
                ...
                if (!this.isDropDown()) {
                    ...
                    to = parseFloat(to).toFixed(fixed);
                    if (!fixed) {
                        var toValue = $.mage.amShopbyFilterAbstract.prototype.fixPriceForCurrency(parseFloat(to), 'to', rate),
                            fromValue = $.mage.amShopbyFilterAbstract.prototype.fixPriceForCurrency(parseFloat(from), 'from', rate),
            ...
            filters.each(function (index, filter) {
                ...
                switch (type) {
                    ...
                    case 'INPUT' :
                        ...
                        } else if ($(filter).attr("type") == 'hidden' && filter.value == value) {
                            filter.value = "";
                        }
                        break;
                }
